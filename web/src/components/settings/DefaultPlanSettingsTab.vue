...
const sortedBagSeeds = computed(() => {
  const byId = new Map(props.bagSeeds.map(seed => [Number(seed.seedId), seed]))
...
function resetBagSeedPriority() {
  strategySettings.value.bagSeedPriority = props.bagSeeds.map(seed => Number(seed.seedId))
}

function moveBagSeed(seedId: number, direction: -1 | 1) {
  const list = strategySettings.value.bagSeedPriority.map(id => Number(id))
  const index = list.indexOf(Number(seedId))
  ...
  list[target] = current
  strategySettings.value.bagSeedPriority = list
}

function removeBagSeedPriority(seedId: number) {
  strategySettings.value.bagSeedPriority = strategySettings.value.bagSeedPriority
    .map(id => Number(id))
    .filter(id => id !== Number(seedId))
}

function reorderBagSeeds(seedIds: number[]) {
  strategySettings.value.bagSeedPriority = seedIds.map(id => Number(id)).filter(id => id > 0)
}
