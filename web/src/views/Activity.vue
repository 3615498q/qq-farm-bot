...
const SHOW_YULU_ACTIVITY = Date.now() < YULU_END
...
const yuluPanelRef = ref<InstanceType<typeof YuluActivityPanel> | null>(null)
...
const sectionTabs = computed<ActivitySection[]>(() => [
  ...
  ...(SHOW_QINGMEI_ACTIVITY
    ...
    : []),
  ...(SHOW_YULU_ACTIVITY
    ? [{ key: 'yulu' as const, label: '雨落成诗', icon: 'i-carbon-rain-drop', count: 0 }]
    : []),
])
...
async function refreshAll() {
  ...
  if (activeSection.value === 'yulu')
    await yuluPanelRef.value?.loadYulu()
}
  ...
  <section class="space-y-4">
    ...
    <template v-else>
      ...
      <div v-else class="min-w-0 space-y-4">
        ...
        <YuluActivityPanel
          v-else-if="SHOW_YULU_ACTIVITY && activeSection === 'yulu'"
          ref="yuluPanelRef"
