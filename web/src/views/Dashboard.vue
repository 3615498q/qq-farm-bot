...
function formatLogLine(log: any) {
  ...
  const tag = log.tag || '系统'
  const event = log.meta?.event ? `[${getEventLabel(log.meta.event)}] ` : ''
  const recovered = log.recovered ? '（已恢复）' : ''
  const msg = log.msg || ''
  return `[${time}] [${tag}] ${event}${msg}${recovered}`
}

const logText = computed(() => {
  if (!allLogs.value.length)
    return ''
  return allLogs.value.map(formatLogLine).join('\n')
})
...
const events = [
  ...
  { label: '雨落自动采集', value: 'yulu_auto_collect' },
]
...
async function refresh(forceReloadLogs = false) {
  ...
  if (!realtimeConnected.value || forceReloadLogs || !(statusAccountLogs.value?.length)) {
    await statusStore.fetchAccountLogs(currentAccountId.value)
  }

  if (forceReloadLogs || hasActiveLogFilter.value || !realtimeConnected.value || !(statusLogs.value?.length)) {
...
watch(logText, () => {
  nextTick(() => {
    if (logTextarea.value && autoScroll.value)
  ...
  <div ref="panelEl" class="flex flex-col gap-2 pt-1 md:pt-2 overflow-x-hidden" @touchstart="onSwipeStart" @touchend="onSwipeEnd">
    ...
    <div v-show="activeTab === 'overview'" class="flex flex-1 flex-col items-stretch gap-5 md:flex-row">
      ...
      <div class="flex flex-1 flex-col gap-5 md:w-3/4">
        <div class="overview-card flex flex-1 flex-col p-5 md:overflow-hidden">
          ...
          <textarea
            ref="logTextarea"
            class="log-textarea ui-subtle-panel max-h-[50vh] min-h-[220px] w-full flex-1 resize-none rounded-lg p-4 text-sm leading-relaxed font-mono"
            readonly
            :value="logText"
            placeholder="暂无日志。运行账号后，这里会持续追加巡查、种植、任务和出售记录。"
            @scroll="onLogScroll"
          />
        </div>
      </div>
    ...
    <div v-show="activeTab === 'strategy'" class="h-full">
      <StrategySettingsTab
        ...
        @reorder-bag-seeds="reorderBagSeeds"
