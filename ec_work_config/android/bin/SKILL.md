...
- **必填**：`-i/--path` 本地图像路径
- **可选**：
  - `-t/--ocr-type`：`paddleOcrNcnnV5|paddleOcrOnnxV4|paddleOcrOnnxV5|paddleOcrOnnxV6|ocrLite`（默认 `paddleOcrNcnnV5`；`paddleOcrOnnxV6` 需设备 API≥26，默认 small 模型）
  - `-P/--padding`：默认 `32`
...
$EC ocr-local-image -m app -i a.jpg -t paddleOcrOnnxV6 -P 48 -X 960 -R
```
...
- **可选**（同 `ocr-local-image`，但不需要 `--path`）：
  - `-t/--ocr-type`（默认 `paddleOcrNcnnV5`；含 `paddleOcrOnnxV6`，需 API≥26）
  - `-P/--padding`（默认 `32`）
  - `-X/--max-side-len`（默认 `640`）
  - `-R/--release`（默认 `false`）
...
$EC ocr-screen -m app -t paddleOcrOnnxV6 -P 48 -X 960 -R
```
