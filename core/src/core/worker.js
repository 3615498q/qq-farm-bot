...
const { startYuluAutoCollectTimer, stopYuluAutoCollectTimer, syncYuluAutoCollectTimer } = require('../services/yulu-scheduler');
...
function applyRuntimeConfig(config, syncStatusAfter = false) {
    ...
    if (loginReady) {
        ...
        if (hasAutomation) {
            ...
            const prevYuluAuto = !!(prevAuto && prevAuto.yulu_auto_collect);
            const newYuluAuto = !!(newAuto && newAuto.yulu_auto_collect);
            if (newYuluAuto && !prevYuluAuto) {
                workerScheduler.setTimeoutTask('yulu_auto_collect_start', 1000, () => {
                    startYuluAutoCollectTimer();
                });
            }
            else if (!newYuluAuto && prevYuluAuto) {
...
async function startBot(config) {
    ...
    const onReady = async () => {
        ...
        if (getAutomation().yulu_auto_collect) {
            workerScheduler.setTimeoutTask('yulu_auto_collect_start', 3000, () => {
                startYuluAutoCollectTimer();
            });
        }
...
async function handleApiCall(msg) {
    ...
    try {
        switch (method) {
            ...
            case 'getYuluStatus': {
                const { getYuluStatus } = require('../services/yulu-activity');
                result = await getYuluStatus();
                break;
            }
            case 'openYuluItem': {
                const { openYuluItem } = require('../services/yulu-activity');
                result = await openYuluItem(Number(args[0]) || 0);
                break;
            }
            case 'mutateYulu': {
                const { mutateYulu } = require('../services/yulu-activity');
                result = await mutateYulu();
                break;
            }
            case 'useYuluItem': {
                const { useYuluItem } = require('../services/yulu-activity');
                const options = args[3] && typeof args[3] === 'object' ? args[3] : {};
                result = await useYuluItem(
                    Number(args[0]) || 0,
                    Number(args[1]) || 0,
            ...
            case 'claimYuluResearch': {
                const { claimYuluResearch } = require('../services/yulu-activity');
                result = await claimYuluResearch(Number(args[0]) || 0);
                break;
            }
            case 'exchangeYuluBottle': {
                const { exchangeYuluBottle } = require('../services/yulu-activity');
                result = await exchangeYuluBottle();
                break;
            }
