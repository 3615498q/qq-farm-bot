...
const (
	...
	oauthAttemptTimeout = 15 * time.Second
	oauthMaxAttempts      = 3
)
...
func oauthCallbackURL(code string) string {
	return callbackURL + "?" + url.Values{
		"login_type": {"WX"},
		"code":       {code},
		"state":      {"web"},
	}.Encode()
...
func (c *Client) exchangeOAuthCode(ctx context.Context, client *http.Client, code string) error {
	cb := oauthCallbackURL(code)
	var lastErr error
	for attempt := 1; attempt <= oauthMaxAttempts; attempt++ {
		attemptCtx, cancel := context.WithTimeout(ctx, oauthAttemptTimeout)
		req, err := http.NewRequestWithContext(attemptCtx, http.MethodGet, cb, nil)
		if err != nil {
			cancel()
			return err
		}
		...
		if err != nil {
			lastErr = err
			if isRetryableNetErr(err) && attempt < oauthMaxAttempts {
	...
	return fmt.Errorf("OAuth callback failed")
}
...
func (c *Client) GetLoginBuffer(ctx context.Context, sess *Session) (protocol.LoginBufferResult, error) {
	...
	if sess.LoginBuffer != "" && sess.Credentials != nil {
		result := protocol.LoginBufferResult{LoginBuffer: sess.LoginBuffer, Credentials: *sess.Credentials}
		sess.mu.Unlock()
		return result, nil
	}
	if sess.AuthorizeCode == "" {
	...
	if err := c.exchangeOAuthCode(ctx, httpClient, code); err != nil {
		return protocol.LoginBufferResult{}, err
	}
	creds, err := credentialsFromJar(jar, oauthCallbackURL(code))
	if err != nil {
		return protocol.LoginBufferResult{}, err
	}
